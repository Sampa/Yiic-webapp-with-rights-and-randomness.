...
class YumUserIdentity extends CUserIdentity {
	...
	public function authenticateFacebook() {
		...
		$profile =
			YumProfile::model()->findByAttributes(array('facebook_id' => $fb_uid));
	...
	public function authenticateLdap() {
		...
		if ($r =
			ldap_search($ds, $settings->ldap_basedn, '(uid=' . $this->username . ')')
		) {
				...
				if ($user == NULL) {
					if ($settings->ldap_autocreate == 1) {
						...
						if ($settings->ldap_transfer_pw == 1)
							$user->password = YumUser::passwordHash($this->password);
	...
	public function authenticate($without_password = false) {
		...
		if ($without_password)
			...
			YumUser::passwordHash($this->password, $user->password) !== $user->password
		)
